...
for j=1:length(f_vec) % loop over each frequency to be tested
    ...
    % s_gen = serialport('COMY',115200); % opens serial connect at specified baud rate
    ...
    writeline(v,"APPLy:SIN " + int2str(f_vec(j)) + ",1,0.75"); % sending over a wave
    ...
    % Wait initial wiattime before sampling to allow function generator to
    % setup
    pause(waittime);
    flush(s_read);

    readline(s_read); % read 1 line to clear any half-read lines
    
    leftover = ''; % create placeholder for any unread partial lines
    tic; % re-start timer to use for plot throttling
    while flag == 0
        nbytes = s_read.NumBytesAvailable;
        if nbytes == 0
            continue % nothing new yet, check again
        end
 
        raw = [leftover, read(s_read, nbytes, "char")]; % char array of everything available
        ...
        vals = sscanf(chunk, '%f, %f, %f\n');
        vals = vals(1:2*floor(numel(vals)/3)); % drop a stray unpaired value, if any
        Aref_all = vals (1:3:end);
        A_all = vals(2:3:end);
        t_all = vals(3:3:end) / 1E6; % convert microseconds to seconds
        ...
        for k = 1:numel(t_all)
            if (t_all(k) - timer) > dt_set
                ...
                voltage(i) = A_all(k) * 3.3/(2^10-1);
                voltage_ref(i) = Aref_all(k) * 3.3/(2^10 - 1); % convert to full scale voltage
                timer = t_all(k);
                i = i + 1;
    ...
    time = time(1:reps)-time(1); % trim and zero time vector
    % trim excess vector
    voltage = voltage(1:reps); 
    % trim excess vector
    voltage_ref = voltage_ref(1:reps); 
    ...
    xlim([f_min f_max]);
    ylim([0 max([1.25*max(transfer_vec) 2.25])]);
